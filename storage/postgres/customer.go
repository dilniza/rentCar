...
func NewCustomerRepo(db *pgxpool.Pool, log logger.ILogger, redis storage.IRedisStorage) CustomerRepo {
	return CustomerRepo{
		db:     db,
		logger: log,
		redis:  redis,
...
func (c *CustomerRepo) Create(ctx context.Context, customer models.CreateCustomer) (string, error) {
	...
	if err != nil {
		c.logger.Error("failed to marshal customer data for Redis", logger.Error(err))
	} else {
		err = c.redis.SetX(ctx, "customer_id:"+id, string(customerJSON), time.Minute*2)
		if err != nil {
			c.logger.Error("failed to save create customer data in Redis", logger.Error(err))
		}
	}
...
func (c *CustomerRepo) CheckEmailExists(ctx context.Context, email string) (bool, error) {
	var exists bool
	query := `SELECT EXISTS (SELECT email FROM customers WHERE email = $1 AND deleted_at = 0)`
	err := c.db.QueryRow(ctx, query, email).Scan(&exists)
	if err != nil {
		c.logger.Error("failed to check email existence", logger.Error(err))
		return false, err
	}
	return exists, nil
}
...
func (c *CustomerRepo) Update(ctx context.Context, customer models.UpdateCustomer, id string) (string, error) {
	...
	err = c.redis.Del(ctx, "customer_id:"+id)
	if err != nil {
		c.logger.Error("failed to delete customer data from Redis", logger.Error(err))
	}
...
func (c *CustomerRepo) GetCustomerCars(ctx context.Context, name string, id string, boolean bool) (models.GetCustomerCarsResponse, error) {
	...
	for rows.Next() {
		...
		customerCar.Duration, err = pkg.Duration(fromDate.String, toDate.String)
		if err != nil {
